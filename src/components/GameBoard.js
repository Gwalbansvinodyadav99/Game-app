...
const GameBoard = () => {
  ...
  const handleTileClick = (id) => {
    ...
    if (!clickedTile.flipped && flippedTiles.length < 2) {
      ...
      const allMatched =
        matchedTiles.length + newFlippedTiles.length === tiles.length;

      if (newFlippedTiles.length === 2) {
        if (checkForMatch(newFlippedTiles)) {
          console.log(matchedTiles);
          console.log(newFlippedTiles);
          const updatedMatchedTiles = newFlippedTiles.map((tile) => ({
            ...tile,
            matched: true,
            flipped: true,
          }));
          ...
          const updatedTiles = tiles.map((tile) => {
            if (
              newFlippedTiles.find((flippedTile) => flippedTile.id === tile.id)
            ) {
          ...
          setTimeout(() => {
            const updatedTiles = tiles.map((tile) => {
              if (
                newFlippedTiles.find(
                  (flippedTile) => flippedTile.id === tile.id
                )
              ) {
  ...
  const checkForMatch = (flippedTiles) => {
    return (
      ...
      flippedTiles[0].emoji === flippedTiles[1].emoji
    );
  };
  const userName = localStorage.getItem("userName");
  return (
